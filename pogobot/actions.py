...
class KeyboardProbe:
    """`dumpsys input_method` behind a TTL cache.

    The window-manager check was the one thing v1 got right - it is exact, where pixel
    heuristics were not - but it called it every loop iteration at a measured 0.08-0.11 s
    per call on this device, which alone capped the loop at roughly 9 Hz. Caching turns
    it into a background-priced signal, and a failed probe reports UNKNOWN rather than
    False so a dead adb cannot masquerade as "no keyboard is up".
    """

    def __init__(self, adb: str = "adb", serial: Optional[str] = None,
                 ttl: float = 2.0, timeout: float = 0.4):
        self.adb = adb
        self.serial = serial
        self.ttl = ttl
        self.timeout = timeout
        self._value: Tristate = Tristate.UNKNOWN
    ...
    def visible(self, now: Optional[float] = None, force: bool = False) -> Tristate:
        ...
        try:
            proc = subprocess.run(
                _adb_argv(self.adb, self.serial, "shell", "dumpsys", "input_method"),
                capture_output=True, timeout=self.timeout, check=False,
            )
        except (OSError, subprocess.SubprocessError):
            self._value = Tristate.UNKNOWN
            return self._value
        if proc.returncode != 0:
            self._value = Tristate.UNKNOWN
            return self._value
        shown = "mInputShown=true" in proc.stdout.decode(errors="replace")
        self._value = Tristate.TRUE if shown else Tristate.FALSE
        return self._value
